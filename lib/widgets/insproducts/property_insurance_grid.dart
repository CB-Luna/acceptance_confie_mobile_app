...
class PropertyInsuranceGrid extends StatefulWidget {
  const PropertyInsuranceGrid({super.key});

  @override
  State<PropertyInsuranceGrid> createState() => _PropertyInsuranceGridState();
}
...
class _PropertyInsuranceGridState extends State<PropertyInsuranceGrid> {
            ...
            children: [
              ...
              GridView.count(
                ...
                children: [
                  _buildInsuranceItem(
                    context,
                    context.translate('propertyInsurance.renters'),
                    'renters',
                  ),
                  _buildInsuranceItem(
                    context,
  ...
  Future<void> _handleInsurance(
    ...
    final overlay = LoadingView.showOverlay(
      context,
      message: context.translate('propertyInsurance.processing'),
  ...
  Future<void> _handleHomeownersInsurance(BuildContext context) async {
    await _handleInsurance(context, 'homeowners');
  }

  Future<void> _handleRentersInsurance(BuildContext context) async {
    await _handleInsurance(context, 'renters');
  }
  ...
  Future<void> _showWebPageDialog(
    ...
    if (!hasBeenShown && context.mounted) {
      final result = await CustomDialog.show(
        context: context,
        title: context.translate('propertyInsurance.location.webDialogTitle'),
        message: context
            .translate('propertyInsurance.location.webDialogMessage')
            .replaceAll('{0}', placeName)
            .replaceAll('{1}', stateAbbreviation),
        positiveButtonText:
            context.translate('propertyInsurance.location.visitWebsite'),
        negativeButtonText:
            context.translate('propertyInsurance.location.cancel'),
      );
